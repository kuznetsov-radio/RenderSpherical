function LineTraceMulti, Nlines, lon_arr, lat_arr, r_arr, Bph, Bth, Br, $
                         lon0_m, lat0_m, r0_m, ds, $
                         l_lon_m, l_lat_m, l_r_m, $
                         lineclosed_m, L_m, Bmax_m, Bavg_m, B1_m, B2_m, fs=fs
;Computes a number of the magnetic field lines in the spherical coordinate 
;system.
...
;
;Output parameters:
; l_lon_m, l_lat_m, l_r_m - 2D arrays (Nmax, Nlines) specifying the 
;  coordinates of the points along the field lines, double;
;  the longitude (l_lon_m) and latitude (l_lat_m) are in degrees and the 
;  radial distance (l_r_m) is in R_Sun units;
;  the i-th point of j-th line has the coordinates (l_lon_m[i, j], 
;  l_lat_m[i, j], l_r_m[i, j]) (defined only for i<N_m[j]);
;  Nmax is a value chosen to accommodate all results, i.e., Nmax equals
;  or exceeds max(N_m)l
...
; field line exceeds 1000000.
 ...
 common ltm_parms, libname, Nmax
                                           
 if size(libname, /type) eq 0 then begin                     
  dirpath=file_dirname((routine_info('LineTraceMulti', /source, /functions)).path, /mark)
 ...
 while ~done do begin
  l_lon_m=dblarr(Nmax, Nlines)
  l_lat_m=dblarr(Nmax, Nlines)
  l_r_m=dblarr(Nmax, Nlines)
 
  Nlon=n_elements(lon_arr)
  Nlat=n_elements(lat_arr)
  Nr=n_elements(r_arr)
 
  NptsMax=call_external(libname, 'TRACE_MULTI', $
                        Nlines, Nlon, Nlat, Nr, lon_arr, lat_arr, r_arr, $
                        Bph, Bth, Br, lon0_m, lat0_m, r0_m, ds, fs_on, Nmax, $
                        N_m, l_lon_m, l_lat_m, l_r_m, lineclosed_m, $
                        l_m, Bmax_m, Bavg_m, B1_m, B2_m)
                  
  if NptsMax le Nmax then done=1 else Nmax=(NptsMax/1024+1)*1024
 endwhile                
